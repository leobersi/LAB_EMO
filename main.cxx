...
int main() {
 /* ///////////////////////////////////////////////
  ...
  TH2F *correlation = new TH2F("correlation", "distribution of angles", 1000, 0,
                               TMath::Pi(), 1000, 0, 2 * TMath::Pi());
  TH1F *impulse = new TH1F("impulse", "Impulse distribution", 1000, 0, 10);
  TH1F *trasv_imp =
      new TH1F("trasv_imp", "Traverse impulse distribution", 1000, 0, 10);
  TH1F *energy =
      new TH1F("energy", "Energy of Particles distribution", 1000, 0, 10);
  ...
  TH1F *mass3 =
      new TH1F("mass3", "Invariant Mass P and K discord charge", 1000, 0, 10);
  TH1F *mass4 =
      new TH1F("mass4", "Invariant Mass P and K concord charge", 1000, 0, 10);
  TH1F *mass5 =
      new TH1F("mass7", "Invariant Mass couples decayed from K*", 1000, 0, 10);
  TH1F *mass6 =
      new TH1F("mass8", "Invariant Mass all particles generated", 1000, 0, 10);
  ...
  for (int i = 0; i < n_gen; ++i) {
    ...
    for (int j = 0; j < 100 + decays_counter; ++j) {
      ...
      if (x < 0.4) {
        ...
        particle[j].Decay2body(particle[100 + decays_counter],
                               particle[100 + decays_counter + 1]);
        decays_counter++;
        decays_counter++;
      } else {
        particle[j].Set_indexParticle(6);
        ...
        particle[100 + decays_counter + 1].Set_indexParticle(1);
        particle[j].Decay2body(particle[100 + decays_counter + 1],
                               particle[100 + decays_counter + 1]);
        decays_counter++;
        decays_counter++;
      }
      ...
      if (particle[j].Get_indexParticle() >= 0) {
        types->Fill(
            particle[j].Get_indexParticle()); // Filling of generated particles
      }
    }
    ...
    for (int j = 0; j < 100 + decays_counter; ++j) {
      for (int h = j + 1; h < 100 + decays_counter; h++) {
        ...
        if ((particle[j].Get_indexParticle() == 0 &&
             particle[j].Get_indexParticle() == 3) ||
            (particle[j].Get_indexParticle() == 1 &&
             particle[j].Get_indexParticle() == 2)) {
          mass3->Fill(particle[j].InvMass(particle[h])); // filling mass3
        }
        if ((particle[j].Get_indexParticle() == 1 &&
             particle[j].Get_indexParticle() == 3) ||
            (particle[j].Get_indexParticle() == 0 &&
             particle[j].Get_indexParticle() == 2)) {
